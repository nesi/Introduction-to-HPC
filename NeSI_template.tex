...
\frame[t]
{
  \frametitle{Our Facilities}
   ...
   \begin{itemize}%[<+-| alert@+>]
	...
	\item Currently have 1700 CPU cores across 55 hosts.
	\item About 35 TB of memory and 85 TFLOPS (distributed).
...
\frame[t]
{
  \frametitle{Our Facilities}
      ...
      \begin{tabular}{|c|c|c|}
      \hline 
      \textbf{Architecture} & \textbf{Power6}  & \textbf{Barometer} ??\\ 
      \hline 
      Model & P575 &  POWER6 \\ 
      \hline 
      Clock Speed & 4.7 GHz &  GHz  \\ 
      \hline 
      Cache & 32MB & MB  \\ 
      \hline 
      Cores/socket & 16 &   \\ 
...
\frame[t]
{
  \frametitle{What to Expect}
   ...
   \begin{itemize}%[<+-| alert@+>]
	\item Not all work is suitable for our HPC facilities (e.g. interactive work)
   \begin{itemize}%[<+-| alert@+>]
	\item Suitable work includes
...
\frame[t]
{
   ...
   \begin{center}
   \includegraphics[width=240pt]{NeSI_img/Amdahl.png}
   \end{center}
  \end{block}
}
...
\frame[t]
{
   ...
   \begin{center}
   \includegraphics[width=330pt]{NeSI_img/migrate_scalability.png}
   \end{center}
  \end{block}
}
